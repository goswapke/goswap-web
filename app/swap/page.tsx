...
export default function SwapPage() {
  ...
  return (
    <section>
      <h1 className="text-2xl font-semibold mb-4">Swap vehicles</h1>
      <p className="text-sm text-slate-600 mb-6">Contacts & plates are masked until both parties pay the swap fee (demo view).</p>
      <div className="grid md:grid-cols-3 gap-4">
        {items.map(v => (
          <div key={v.id} className="border rounded-lg p-4">
            ...
            <div className="mt-2 text-sm">Plate: {v.plateMasked ?? "K** ***"}</div>
            <p className="mt-2 text-sm text-slate-700">{v.description}</p>
            <a href="/auth/signin" className="mt-3 inline-block text-sm rounded px-3 py-1 border">Match</a>
          </div>
        ))}
      </div>
    </section>
